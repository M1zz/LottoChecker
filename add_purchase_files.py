...
# PBXGroup 섹션에서 LottoChecker 그룹 찾기
group_section = re.search(r'/\* LottoChecker \*/.*?children = \((.*?)\);', content, re.DOTALL)
if not group_section:
    print("Could not find LottoChecker group")
    exit(1)

# PBXSourcesBuildPhase 섹션 찾기
sources_section = re.search(r'/\* Sources \*/.*?isa = PBXSourcesBuildPhase;.*?files = \((.*?)\);', content, re.DOTALL)
